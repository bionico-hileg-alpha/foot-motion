...
from helpers.utilsnn import xavier_init
...
class AutoEncoder(object):
    def __init__(self, input_size, layer_sizes, layer_names, tied_weights=False, optimizer=tf.train.AdamOptimizer(),
        ...
        self.file_writer = tf.summary.FileWriter('/tmp/logs/', self.sess.graph)
        self.sess.run(init)

    def transform(self, X):
        return self.sess.run(self.encoded_x, feed_dict={self.x: X})
